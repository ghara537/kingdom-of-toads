...
class Card:
    ...
    def describe(self) -> str:
        ...
        if self.is_engine:
            ...
            return f"{gate}+{amount} {_kind_label(kind)}{cadence}"
        if self.is_instant:
            kind, amount = self.effect
            return f"+{amount} {_kind_label(kind)} immediately"
...
def deck_composition(player_count: int) -> list[str]:
    """The full multiset of card ids for a game of `player_count` players.

    Unshuffled and in a stable order — the engine shuffles it with the game
    seed so that a given seed always produces the same deck.
    """
...
def deck_size(player_count: int) -> int:
    return len(deck_composition(player_count))
